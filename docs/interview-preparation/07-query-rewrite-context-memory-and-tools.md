...
| Query Rewrite | 补全指代、生成检索变体      | 当前问题、最近 8 条消息（约 4 轮）；必要时附加恢复历史       | 1600       |
| Answer        | 基于证据生成答案和引用      | 长期记忆、摘要、最近 4 轮原文、恢复历史、完整检索证据、工具结果 | 6000       |
...
Answer 的 active window 是最近 4 轮原文（通常约 8 条 user/assistant message）。更早的消息属于 retired rounds 候选区：数据库中不设总轮数上限，但每次摘要最多处理一批消息。历史区域由摘要、近期消息、相关历史和长期记忆组成；检索证据与工具结果不属于历史区域。摘要触发只计算“窗口外且尚未被摘要游标覆盖”的消息，并与历史区域预算比较，不看全部会话字符数。
...
完整维护顺序是：正常情况下在回答保存后压缩 retired rounds 并推进摘要游标；下一次 Pack 构建时使用新摘要，
并依次去重工具结果、淘汰已回答或较旧工具结果、减少 `relevant_history`、裁剪低分证据，
最后才允许裁剪近期消息和长期记忆。若当前 Answer Pack 达到 95% Hard 水位，则本次请求额外把 active window
从最近 4 轮临时收缩为最近 2 轮，并同步调用一次摘要 LLM 压缩较旧 2 轮；这份临时摘要不覆盖数据库中的持久摘要。

例子：M1-M8 是 retired 历史，M9-M16 是最近 4 轮 active 原文。如果摘要、最近窗口、恢复历史和长期记忆
合计达到 70%，系统从 M1 开始选择一批消息，单次最多 16 条，并尽量让占用回落到约 60%。假设本次选到 M1-M8，摘要
游标就指向 M8；下一次 Pack 只放摘要和 M9-M16，不会同时重复放 M1-M8 原文。若本次 Pack 达到 95%，则临时保留 M13-M16，
把 M9-M12 交给本次摘要调用，回答结束后仍按持久摘要流程继续推进。
...
本项目把 Answer 原文窗口定义为最多 4 轮，而不是 4 条消息。正常请求保留这 4 轮；Hard 水位时临时保护最近 2 轮原文，
把 active window 中较早的 2 轮交给一次性摘要。active window 之外的 retired rounds 继续从最早未摘要消息开始分批处理；批次不是固定轮数，
但单次最多 16 条消息。完整消息仍然保存在 PostgreSQL，不会物理删除。
